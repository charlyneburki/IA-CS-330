...
from .noeud_de_decision_adv import NoeudDeDecisionAdv
   ...
   def construit_arbre_recur(self, donnees, attributs, predominant_class,level=0):
    ...
    def get_split(dataset):
        ...
        for index in range(len(dataset[0])-1):
            for row in dataset:
                groups = partitionne(self, dataset,index, row[index], dataset)
    ...
    def try_split(self,attribut, valeur, donnees):
        return None
    
    def iterate_data(self,donnees):
        
        values =[]
        for att,value in donnees.items():
            if attribut == att:
                values.append((att,value))
        return values
        
    def caclule_entropie(self, donnees, split_candidat):
        entropie = self.h_C_A(donnees, split_candidat[0], split_candidat[1])
        
        return entropie
    
   # def find_min_entropie(self,donnees)
